...
```{r cache=TRUE}
set.seed(2)
# Set tuning parameters (25 as maximum number of terms taken since only 18 predictors are used)
...
```{r}
set.seed(2)
knn_grid = expand.grid(k = seq(5, 13, by = 1))
...
```{r cache=TRUE}
set.seed(2)
# squared root of 18 is around 4.2, therefore 3 to 5 predictors are considered in tree splits in this case
rf_grid = expand.grid(mtry = c(3:6))
rf_model = train(predictors_train, response_train,
                  method = "rf",
                 preProcess = c("center", "scale"),
                 trControl = ctrl_1,
                 tuneGrid = rf_grid)
